...
### 2025-06-21 修复充值和提现功能

修复了充值和提现功能中的存储过程参数错误问题。具体修改如下：

1. 将MySQL存储过程`GenerateTransactionId`的参数类型从`OUT`改为`INOUT`，解决了"Parameter number 1 is not an OUT parameter"的错误。
2. 在`TransactionDAO`中调用存储过程时，增加了`setString`方法设置参数初始值，以适应`INOUT`参数的要求。

这些修改解决了充值和提现功能中出现的500错误问题。
